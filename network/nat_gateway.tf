resource "aws_eip" "vpc_eip_1a" {
  ...
  tags = {
    Name = format("%s-nat-eip-1a", var.project_name)
  }
}
...
resource "aws_eip" "vpc_eip_1b" {
  ...
  tags = {
    Name = format("%s-nat-eip-1b", var.project_name)
  }
}
...
resource "aws_eip" "vpc_eip_1c" {
  ...
  tags = {
    Name = format("%s-nat-eip-1c", var.project_name)
  }
}

resource "aws_nat_gateway" "vpc_nat_gw_1a" {
...
resource "aws_nat_gateway" "vpc_nat_gw_1b" {
...
resource "aws_nat_gateway" "vpc_nat_gw_1c" {
